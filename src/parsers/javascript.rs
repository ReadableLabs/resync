...
pub fn get_symbol_pair(input: Span) -> IResult<Span, (SymbolPosition, SymbolPosition)> {
    ...
    // check if function is within those lines
    let (_input, (fun_type, comment_type)) = match get_symbol_type(new_lines.as_str()) {
        ...
        // Free means there is no function underneath
        Err(_) => ("", (FunType::Empty, CommentType::Free))
    };

    // get function under comment
    let (input, (fun_start, fun_end)) = match comment_type {
        ...
        // get next 2 lines under comment
        CommentType::Free => {
...
/// Gets the type of symbol - eg arrow function or function. Meant to be used with two lines passed
/// in, and will return empty if a function is not within those two lines
